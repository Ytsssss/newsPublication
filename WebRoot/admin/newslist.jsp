...
<html lang="en">
  ...
  <body class="">
    ...
    <%	 CommDAO dao = new CommDAO(); 
	 ...
	 String url = "/newspubsys/admin/newslist.jsp?1=1";
			...
			<%}%>
				<div class="btn-toolbar">
					<div class="search-well">
						<form action="admin/newslist.jsp?f=f" method="post" class="form-inline">
...
<div class="well">
   ...
   ArrayList<HashMap> list=(ArrayList)bean.getCollection();
   	for(HashMap map:list){
   		HashMap mmmm = dao.select("select * from ppinfo where id="+map.get("type")).get(0);
   		HashMap mbmap = dao.select("select * from member where id="+map.get("mid")).get(0);
	    %>
        <tr>
          <td><a href="/newspubsys/newsx.jsp?id=<%=map.get("id") %>" target="_blank"><%=map.get("title") %></a></td>
          <td><%=mmmm.get("ppname") %></td>
          <td><%=mbmap.get("tname") %></td>
          <td><%=map.get("savetime") %></td>
          <td><%=map.get("shstatus") %></td>
          <td>
              <a href="admin/newsedit.jsp?id=<%=map.get("id") %>" title="审核"><i class="icon-pencil"></i></a>
