...
<html lang="en">
  ...
  <body class="">
    ...
    <%	 CommDAO dao = new CommDAO(); 
     String sql = "select * from projects where 1=1 and delstatus='0' ";
	 String url = "/newspubsys/admin/projects.jsp?1=1";
...
<div class="well">
   ...
   ArrayList<HashMap> list=(ArrayList)bean.getCollection();
   	for(HashMap map:list){
   		HashMap csmap = dao.select("select * from member where id="+map.get("uid")).get(0);
	    %>
        <tr>
          <td><%=map.get("pno") %></td>
          <td><%=map.get("title") %></td>
          <td><%=map.get("jlmoney") %></td>
          <td><%=csmap.get("tname") %></td>
          <td><%=map.get("savetime") %></td>
          <td>
              <a href="admin/projectedit.jsp?id=<%=map.get("id") %>"><i class="icon-pencil"></i></a>
              <a href="admin/projects.jsp?did=<%=map.get("id") %>" ><i class="icon-remove"></i></a>
          </td>
        </tr>
        <%} %>
      </tbody>
