...
<html lang="en">
  ...
  <body class="">
    ...
    <%	 CommDAO dao = new CommDAO(); 
	 String sql = "select * from member where 1=1  and delstatus=0 and utype='厂商'";
	 String url = "/newspubsys/admin/member2list.jsp?1=1";
            ...
            <div class="row-fluid">
            	...
            	<div class="btn-toolbar">
					<div class="search-well">
						<form action="admin/member2list.jsp?f=f" method="post" class="form-inline">
...
<div class="well">
    ...
    <table class="table">
      ...
      <%String did = request.getParameter("did");
	   ...
	   if(tgid!=null)
	   {
			dao.commOper("update member set shstatus='通过' where id="+tgid);
	   }
	   String jjid = request.getParameter("jjid");
	   if(jjid!=null)
	   {
			dao.commOper("update member set shstatus='拒绝' where id="+jjid);
	   }
	   ...
	   for(HashMap map:list){
        ...
        <tr>
          ...
          <%if(map.get("shstatus").equals("待审核")){ %>
         		 <a href="admin/member2list.jsp?tgid=<%=map.get("id") %>">通过</a>
         		 |
         		<a href="admin/member2list.jsp?jjid=<%=map.get("id") %>">拒绝</a>
          <%}else{ %>
          	<%=map.get("shstatus") %>
          <%} %>
          </td>
          <td>
              <a href="admin/member2list.jsp?did=<%=map.get("id") %>" ><i class="icon-remove"></i></a>
          </td>
        </tr>
        <%} %>
      </tbody>
